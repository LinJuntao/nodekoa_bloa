...
let posts = `create table if not exists posts(
     ...
     pv int(40) NOT NULL DEFAULT '0' COMMENT '浏览量',
...
/**数据库操作，执行下面的指令,exports为暴露在外部，可以使用 */
//注册用户
...
exports.findPostByPage = page => {
  let _sql = ` select * from posts  ORDER BY pv DESC  limit ${(page - 1) * 10},10;`;
  return query(_sql);
};
// 查询所有个人用户文章数量
